...
def get(adjacent_nodes,prey_dist_array,cur_prey_dist,pred_dist_array,cur_pred_dist):
            optimal_adjascent_node = adjacent_nodes[0]# Optimal node index
            ...
            for k,i in enumerate(adjacent_nodes):
                ...
                if prey_dist_array[k] < cur_prey_dist and pred_dist_array[k] > cur_pred_dist:
                    ...
                    if prey_dist_array[k] < optimal_prey_distance and pred_dist_array[k] > optimal_pred_distance:
                        ...
                        cycle_flag += 1 # Update the cycle_flag variable to indicate that a higher priority condition has already been satisfied by the optimal_adjascent_node
                    ...
                    if prey_dist_array[k] < optimal_prey_distance and pred_dist_array[k] > optimal_pred_distance and not cycle_flag > 0: # if the cycle_flag is greater than 0, it indicates that the above condition has already been accepted, so an optimal_adjascent_node already exists
                        optimal_adjascent_node = i
                        optimal_prey_distance = prey_dist_array[k]
                        optimal_pred_distance = pred_dist_array[k]
